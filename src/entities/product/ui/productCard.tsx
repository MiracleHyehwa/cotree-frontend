...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <Link to={`/product/${id}`} className="relative bg-background overflow-hidden">
      <div className="relative w-full aspect-[3/4]">
        <img src={image_url} alt={name} className="absolute inset-0 w-full h-full object-cover" />
        {is_green === "Y" && (
          <Badge className="absolute bottom-2 right-2 animate-pulse bg-primary text-white border-none">친환경</Badge>
        )}
      </div>
